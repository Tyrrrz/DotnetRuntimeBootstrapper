...
**.NET Runtime Bootstrapper** is a build plugin that replaces the default application host `exe` file — generated by MSBuild for Windows executables — with a fully featured bootstrapper that can automatically download and install .NET runtime and other missing components required by your application.
