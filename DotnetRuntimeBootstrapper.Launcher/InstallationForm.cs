...
namespace DotnetRuntimeBootstrapper.Launcher
{
    ...
    {
        private readonly BootstrapperInputs _inputs;
        private readonly IRuntimeComponent[] _missingRuntimeComponents;

        public DialogResult Result { get; private set; } = DialogResult.None;

        public InstallationForm(BootstrapperInputs inputs, IRuntimeComponent[] missingRuntimeComponents)
        {
            _inputs = inputs;
        ...
        private void PromptReboot() => InvokeOnUI(() =>
        {
            var result = MessageBox.Show(
                $"You need to restart Windows before you can run {_inputs.TargetApplicationName}. " +
        ...
        {
            Text = $"Missing Dependencies: {_inputs.TargetApplicationName}";
            PictureBox.Image = SystemIcons.Warning.ToBitmap();

            DescriptionLabel.Text =
                $"Your system is missing runtime components required by {_inputs.TargetApplicationName}. " +
